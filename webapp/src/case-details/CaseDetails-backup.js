  ...
  const verificationSteps = React.useMemo(() => [
    {
      ...
      description: 'VIN number, registration documents, ownership verification',
      icon: <CarIcon />,
    ...
    },
    {
      ...
      icon: <InsuranceIcon />,
      status: caseData.insurance && !caseData.insurance.isNewPolicy ? 'success' : 'warning',
    ...
    },
    {
      ...
      icon: <InspectionIcon />,
      status: caseData.images?.length > 0 && caseData.damage ? 'success' : 'warning',
      autoCheck: caseData.images?.length > 0 && caseData.damage,
    },
    {
      id: 'customer_verification',
      title: 'Customer Identity Verification',
      description: 'ID verification, signature collection, contact confirmation',
      icon: <PersonIcon />,
      status: caseData.damage?.signatureStatus === 'Completed' ? 'success' : 'warning',
    ...
    },
    {
      ...
      icon: <PricingIcon />,
      status: caseData.parts?.every(part => part.approved) ? 'success' : 'warning',
    ...
    },
    {
      ...
      icon: <RepairIcon />,
      status: !caseData.requiresCalibration || caseData.calibrationConfirmed ? 'success' : 'warning',
      autoCheck: !caseData.requiresCalibration || caseData.calibrationConfirmed,
    },
    {
      id: 'documentation',
      ...
      <CardContent>
        ...
        <List>
          {verificationSteps.map((step, index) => {
            ...
            const statusColor = step.status === 'success' ? 'success.main' : 'warning.main';
            
            return (
              <ListItem key={step.id} divider={index < verificationSteps.length - 1}>
                <ListItemIcon>
                  <Avatar 
                    sx={{ 
        ...
        {progressPercentage === 100 && (
          <Alert severity="success" sx={{ mt: 2 }}>
            <Typography variant="body2">
              All verification steps completed! Case is ready for final approval.
            </Typography>
          </Alert>
        )}
        ...
        {progressPercentage < 100 && (
          <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
            <Button 
              ...
              size="small"
              onClick={() => {
                const allSteps = new Set(verificationSteps.map(step => step.id));
...
const CaseSection = ({ title, status, icon, children }) => {
  // Auto-expand if status is success (approved)
  const defaultExpanded = status === "success";
        ...
        <Grid item xs={12} lg={8}>
          <Stack spacing={3}>
            ...
            <CaseSection
              ...
              icon={<InsuranceIcon />}
              verificationId="insurance_validation"
              verificationDescription="Active policy verification, coverage confirmation"
              onVerificationChange={handleVerificationChange}
              isVerified={verifiedSteps.has('insurance_validation')}
            >
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  <InfoRow
            ...
            <CaseSection
              ...
              icon={<ReportIcon />}
              verificationId="damage_assessment"
              verificationDescription="Photos uploaded, inspection report, damage evaluation"
              onVerificationChange={handleVerificationChange}
              isVerified={verifiedSteps.has('damage_assessment')}
            >
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
            ...
            {caseData.requiresCalibration && (
              <CaseSection
                ...
                icon={<RepairIcon />}
                verificationId="calibration_check"
                verificationDescription="ADAS systems calibration assessment and scheduling"
                onVerificationChange={handleVerificationChange}
                isVerified={verifiedSteps.has('calibration_check')}
              >
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
        ...
        <Grid item xs={12} lg={4}>
          <Stack spacing={3}>
            ...
            <CaseSection
              ...
              icon={<RepairIcon />}
              verificationId="parts_approval"
              verificationDescription="Cost estimation, parts sourcing, labor cost approval"
              onVerificationChange={handleVerificationChange}
              isVerified={verifiedSteps.has('parts_approval')}
            >
            ...
            <CaseSection
              ...
              icon={<ImageIcon />}
              verificationId="customer_verification"
              verificationDescription="ID verification, signature collection, contact confirmation"
              onVerificationChange={handleVerificationChange}
              isVerified={verifiedSteps.has('customer_verification')}
            >
